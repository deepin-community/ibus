/* property.c generated by valac 0.56.12, the Vala compiler
...
static void ____lambda52_ (PropertyManager* self,
                    const gchar* k,
                    gint s);
static void _____lambda52__iprop_item_property_activate (IPropItem* _sender,
                                                  const gchar* key,
                                                  gint state,
                                                  gpointer self);
                                    ...
                                    GtkMenu* menu)
{
	...
	result = property_manager_create_menu_items_internal (self, _tmp1_, menu);
	return result;
}

static void
____lambda52_ (PropertyManager* self,
               const gchar* k,
               gint s)
{
...
static void
_____lambda52__iprop_item_property_activate (IPropItem* _sender,
                                             const gchar* key,
                                             gint state,
                                             gpointer self)
{
	____lambda52_ ((PropertyManager*) self, key, state);
}
                                             ...
                                             GtkMenu* menu)
{
	...
	while (TRUE) {
		...
		g_debug ("property.vala:47: ins prop = %s", _tmp4_);
		_tmp5_ = i;
		i = _tmp5_ + 1;
		item = NULL;
		_tmp6_ = prop;
		switch (ibus_property_get_prop_type (_tmp6_)) {
			...
			default:
			{
				...
				_tmp30_ = prop;
				g_warning ("property.vala:79: unknown property type %d", (gint) ibus_property_get_prop_type (_tmp30_));
				break;
			}
		}
		_tmp31_ = prop;
		...
		if (_tmp32_ != NULL) {
			...
			_tmp34_ = item;
			g_signal_connect (_tmp34_, "property-activate", (GCallback) _____lambda52__iprop_item_property_activate, self);
		}
		_g_object_unref0 (item);
		_g_object_unref0 (prop);
	}
