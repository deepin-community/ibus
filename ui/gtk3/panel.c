/* panel.c generated by valac 0.56.12, the Vala compiler
...
static void __lambda94_ (Block6Data* _data6_,
                  guint b,
                  guint t);
static void ___lambda94__gtk_status_icon_popup_menu (GtkStatusIcon* _sender,
                                              ...
                                              guint activate_time,
                                              gpointer self);
static void __lambda95_ (Block6Data* _data6_);
...
static void ___lambda61_ (Block8Data* _data8_,
                   gint id);
...
static void ____lambda93_ (Block9Data* _data9_,
                    GtkMenuItem* item);
...
static gboolean __lambda106_ (Block10Data* _data10_);
                 ...
                 gboolean is_wayland_im)
{
	...
	PropertyManager* _tmp5_;
	PropertyManager* _tmp6_;
	...
	Switcher* _tmp14_;
	Switcher* _tmp15_;
	PropertyPanel* _tmp17_;
	PropertyPanel* _tmp18_;
	...
	_tmp5_ = property_manager_new ();
	_property_manager_unref0 (self->priv->m_property_manager);
	self->priv->m_property_manager = _tmp5_;
	_tmp6_ = self->priv->m_property_manager;
	g_signal_connect_object (_tmp6_, "property-activate", (GCallback) ___lambda84__property_manager_property_activate, self, 0);
	...
	g_signal_connect_object (_tmp8_, "page-up", (GCallback) ___lambda96__candidate_panel_page_up, self, 0);
	_tmp9_ = self->priv->m_candidate_panel;
	g_signal_connect_object (_tmp9_, "page-down", (GCallback) ___lambda97__candidate_panel_page_down, self, 0);
	_tmp10_ = self->priv->m_candidate_panel;
	g_signal_connect_object (_tmp10_, "cursor-up", (GCallback) ___lambda98__candidate_panel_cursor_up, self, 0);
	_tmp11_ = self->priv->m_candidate_panel;
	g_signal_connect_object (_tmp11_, "cursor-down", (GCallback) ___lambda99__candidate_panel_cursor_down, self, 0);
	_tmp12_ = self->priv->m_candidate_panel;
	g_signal_connect_object (_tmp12_, "candidate-clicked", (GCallback) ___lambda100__candidate_panel_candidate_clicked, self, 0);
	_tmp13_ = self->priv->m_candidate_panel;
	g_signal_connect_object (_tmp13_, "realize-surface", (GCallback) ___lambda101__candidate_panel_realize_surface, self, 0);
	_tmp14_ = switcher_new ();
	g_object_ref_sink (_tmp14_);
	_g_object_unref0 (self->priv->m_switcher);
	self->priv->m_switcher = _tmp14_;
	_tmp15_ = self->priv->m_switcher;
	g_signal_connect_object (_tmp15_, "realize-surface", (GCallback) ___lambda102__switcher_realize_surface, self, 0);
	panel_bind_switch_shortcut (self);
	if (self->priv->m_switcher_delay_time >= 0) {
		Switcher* _tmp16_;
		_tmp16_ = self->priv->m_switcher;
		switcher_set_popup_delay_time (_tmp16_, (guint) self->priv->m_switcher_delay_time);
	}
	_tmp17_ = property_panel_new ();
	g_object_ref_sink (_tmp17_);
	_g_object_unref0 (self->priv->m_property_panel);
	self->priv->m_property_panel = _tmp17_;
	_tmp18_ = self->priv->m_property_panel;
	g_signal_connect_object (_tmp18_, "property-activate", (GCallback) ___lambda103__property_panel_property_activate, self, 0);
...
panel_is_kde (void)
{
	...
	_tmp9_ = desktop;
	if (_tmp9_ == NULL) {
		g_warning ("panel.vala:339: %s", "XDG_CURRENT_DESKTOP is not exported in your desktop " "session.");
	}
	g_warning ("panel.vala:342: %s", "If you launch KDE5 on xterm, " "export XDG_CURRENT_DESKTOP=KDE before launch KDE5.");
	result = FALSE;
	return result;
}
...
static void
__lambda94_ (Block6Data* _data6_,
             guint b,
             guint t)
{
	Panel* self;
...
static void
__lambda95_ (Block6Data* _data6_)
{
	Panel* self;
	GtkMenu* _tmp0_;
	GtkMenu* _tmp1_;
...
panel_init_status_icon_menu (Panel* self)
{
	...
	_tmp3_ = display;
	if (_tmp3_ == NULL) {
		g_warning ("panel.vala:404: No Gdk.X11.Display");
		_g_object_unref0 (display);
	...
	if (_tmp8_) {
		...
		_g_object_unref0 (_data6_->window);
		_data6_->window = _tmp15_;
		_tmp16_ = _data6_->window;
		if (_tmp16_ == NULL) {
			...
			_g_object_unref0 (_data6_->window);
			_data6_->window = (GdkWindow*) _tmp18_;
		}
	}
	_tmp19_ = _data6_->window;
	if (_tmp19_ == NULL) {
		g_warning ("panel.vala:423: StatusIcon does not have GdkWindow");
		_g_object_unref0 (display);
		block6_data_unref (_data6_);
		_data6_ = NULL;
		return;
	...
	_data6_->area = _tmp21_;
	orient = _tmp22_;
	win_x = 0;
	win_y = 0;
	_tmp23_ = _data6_->window;
	...
	_tmp27_ = g_signal_connect_data (_tmp26_, "popup-menu", (GCallback) ___lambda94__gtk_status_icon_popup_menu, block6_data_ref (_data6_), (GClosureNotify) block6_data_unref, 0);
	self->priv->m_popup_menu_id = _tmp27_;
	_tmp28_ = self->priv->m_status_icon;
	_tmp29_ = g_signal_connect_data (_tmp28_, "activate", (GCallback) ___lambda95__gtk_status_icon_activate, block6_data_ref (_data6_), (GClosureNotify) block6_data_unref, 0);
                            ...
                            GList* engines)
{
			...
			while (TRUE) {
				...
				if (_tmp35_ != NULL) {
					...
					if (_tmp40_ == NULL) {
						...
						_tmp41_ = layout;
						_tmp42_ = variant;
						g_warning ("panel.vala:556: Fallback %s(%s) to us layout.", _tmp41_, _tmp42_);
						_tmp43_ = us_engine;
	...
	if (g_list_length (_tmp45_) == ((guint) 0)) {
		g_warning ("panel.vala:562: Not found IBus XKB engines from the session.");
	}
	result = xkb_engines;
...
panel_set_xkb_icon_rgba (Panel* self)
{
	...
	if (!gdk_rgba_parse (&rgba, _tmp3_)) {
		...
		GdkRGBA _tmp5_ = {0};
		_tmp4_ = spec;
		g_warning ("panel.vala:743: invalid format of xkb-icon-rgba: %s", _tmp4_);
...
panel_check_wayland (Panel* self)
{
	...
	__catch0_g_error:
	{
		...
		_tmp18_ = message;
		g_warning ("panel.vala:890: %s", _tmp18_);
	}
	__finally0:
	if (G_UNLIKELY (_inner_error0_ != NULL)) {
                              ...
                              IBusEngineDesc* engine)
{
	...
	if (g_hash_table_size (_tmp0_) >= ((guint) 200)) {
		GHashTable* _tmp1_;
		g_warning ("panel.vala:1009: Contexts by windows are too much counted!");
                  ...
                  IBusEngineDesc* engine)
{
	...
	if (!ibus_bus_set_global_engine (_tmp0_, _tmp1_)) {
		...
		g_warning ("panel.vala:1034: Switch engine to %s failed.", _tmp2_);
		return;
	}
	_tmp3_ = g_strdup ("");
                            ...
                            gboolean reverse)
{
	...
	if (_tmp7_) {
		...
		if (i < 0) {
			g_debug ("panel.vala:1106: switch cancelled");
		} else {
			if (i == 0) {
				g_debug ("panel.vala:1108: do not have to switch");
			} else {
                           ...
                           gint index)
{
	...
	_vala_array_add22 (&_data7_->names, &_data7_->names_length1, &_data7_->_names_size_, _tmp3_);
	self->priv->m_preload_engines_id = g_timeout_add_full (G_PRIORITY_DEFAULT, PANEL_PRELOAD_ENGINES_DELAY_TIME, ___lambda62__gsource_func, block7_data_ref (_data7_), block7_data_unref);
	block7_data_unref (_data7_);
	_data7_ = NULL;
}
...
static void
___lambda61_ (Block8Data* _data8_,
              gint id)
{
	Panel* self;
                      ...
                      gint order_names_length1)
{
	...
	if (_tmp22__length1 < _tmp23__length1) {
		...
		g_signal_connect_data ((GtkDialog*) _tmp41_, "response", (GCallback) ____lambda61__gtk_dialog_response, block8_data_ref (_data8_), (GClosureNotify) block8_data_unref, 0);
...
panel_show_setup_dialog (Panel* self)
{
	...
	__catch0_g_spawn_error:
	{
		...
		g_warning ("panel.vala:1377: Execute %s failed! %s", _tmp7_, _tmp9_);
		self->priv->m_setup_pid = (GPid) 0;
		_g_error_free0 (e);
	}
	__finally0:
                           ...
                           gboolean use_x11)
{
	...
	if (_tmp3_) {
		...
		display_backup = _tmp5_;
		_tmp6_ = gdk_display_manager_get ();
		_tmp7_ = binding_common_get_xdisplay ();
		_tmp8_ = G_TYPE_CHECK_INSTANCE_CAST (_tmp7_, gdk_display_get_type (), GdkDisplay);
		gdk_display_manager_set_default_display (_tmp6_, _tmp8_);
		_g_object_unref0 (_tmp8_);
	}
	_tmp9_ = (GtkMenu*) gtk_menu_new ();
	g_object_ref_sink (_tmp9_);
	_g_object_unref0 (self->priv->m_sys_menu);
	self->priv->m_sys_menu = _tmp9_;
	_tmp10_ = (GtkMenuItem*) gtk_menu_item_new_with_label (_ ("Preferences"));
	g_object_ref_sink (_tmp10_);
	_g_object_unref0 (item);
	item = _tmp10_;
	_tmp11_ = item;
	g_signal_connect_object (_tmp11_, "activate", (GCallback) ___lambda87__gtk_menu_item_activate, self, 0);
	_tmp12_ = self->priv->m_sys_menu;
	_tmp13_ = item;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp12_, _tmp13_);
	_tmp14_ = (GtkMenuItem*) gtk_menu_item_new_with_label (_ ("Emoji Choice"));
	g_object_ref_sink (_tmp14_);
	_g_object_unref0 (item);
	item = _tmp14_;
	_tmp15_ = item;
	g_signal_connect_object (_tmp15_, "activate", (GCallback) ___lambda89__gtk_menu_item_activate, self, 0);
	_tmp16_ = self->priv->m_sys_menu;
	_tmp17_ = item;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp16_, _tmp17_);
	_tmp18_ = (GtkMenuItem*) gtk_menu_item_new_with_label (_ ("About"));
	g_object_ref_sink (_tmp18_);
	_g_object_unref0 (item);
	item = _tmp18_;
	_tmp19_ = item;
	g_signal_connect_object (_tmp19_, "activate", (GCallback) ___lambda90__gtk_menu_item_activate, self, 0);
	_tmp20_ = self->priv->m_sys_menu;
	_tmp21_ = item;
	...
	_tmp24_ = _tmp23_;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp22_, (GtkMenuItem*) _tmp24_);
	_g_object_unref0 (_tmp24_);
	_tmp25_ = (GtkMenuItem*) gtk_menu_item_new_with_label (_ ("Restart"));
	g_object_ref_sink (_tmp25_);
	_g_object_unref0 (item);
	item = _tmp25_;
	_tmp26_ = item;
	g_signal_connect_object (_tmp26_, "activate", (GCallback) ___lambda91__gtk_menu_item_activate, self, 0);
	_tmp27_ = self->priv->m_sys_menu;
	_tmp28_ = item;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp27_, _tmp28_);
	_tmp29_ = (GtkMenuItem*) gtk_menu_item_new_with_label (_ ("Quit"));
	g_object_ref_sink (_tmp29_);
	_g_object_unref0 (item);
	item = _tmp29_;
	_tmp30_ = item;
	g_signal_connect_object (_tmp30_, "activate", (GCallback) ___lambda92__gtk_menu_item_activate, self, 0);
	_tmp31_ = self->priv->m_sys_menu;
	_tmp32_ = item;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp31_, _tmp32_);
	_tmp33_ = self->priv->m_sys_menu;
	gtk_widget_show_all ((GtkWidget*) _tmp33_);
	_tmp34_ = display_backup;
	if (_tmp34_ != NULL) {
		GdkDisplayManager* _tmp35_;
		GdkDisplay* _tmp36_;
		_tmp35_ = gdk_display_manager_get ();
		_tmp36_ = display_backup;
		gdk_display_manager_set_default_display (_tmp35_, _tmp36_);
	}
	_tmp37_ = self->priv->m_sys_menu;
...
static void
____lambda93_ (Block9Data* _data9_,
               GtkMenuItem* item)
{
	Panel* self;
	self = _data9_->self;
                            ...
                            gboolean use_x11)
{
	...
	if (_tmp0_) {
		GdkDisplay* _tmp1_;
		GdkDisplay* _tmp2_;
		GdkDisplayManager* _tmp3_;
		GdkX11Display* _tmp4_;
		GdkDisplay* _tmp5_;
		_tmp1_ = gdk_display_get_default ();
		_tmp2_ = _g_object_ref0 (_tmp1_);
		_g_object_unref0 (display_backup);
		display_backup = _tmp2_;
		_tmp3_ = gdk_display_manager_get ();
		_tmp4_ = binding_common_get_xdisplay ();
		_tmp5_ = G_TYPE_CHECK_INSTANCE_CAST (_tmp4_, gdk_display_get_type (), GdkDisplay);
		gdk_display_manager_set_default_display (_tmp3_, _tmp5_);
		_g_object_unref0 (_tmp5_);
	}
	_tmp6_ = (GtkMenu*) gtk_menu_new ();
	g_object_ref_sink (_tmp6_);
	_g_object_unref0 (self->priv->m_ime_menu);
	self->priv->m_ime_menu = _tmp6_;
	_tmp7_ = self->priv->m_property_manager;
	_tmp8_ = self->priv->m_ime_menu;
	property_manager_create_menu_items (_tmp7_, _tmp8_);
	_tmp9_ = self->priv->m_ime_menu;
	_tmp10_ = (GtkSeparatorMenuItem*) gtk_separator_menu_item_new ();
	g_object_ref_sink (_tmp10_);
	_tmp11_ = _tmp10_;
	gtk_menu_shell_append ((GtkMenuShell*) _tmp9_, (GtkMenuItem*) _tmp11_);
	...
	_tmp12__length1 = self->priv->m_engines_length1;
	{
		...
		gint engine_it = 0;
		engine_collection = _tmp12_;
		engine_collection_length1 = _tmp12__length1;
		for (engine_it = 0; engine_it < engine_collection_length1; engine_it = engine_it + 1) {
			IBusEngineDesc* _tmp13_;
			IBusEngineDesc* engine = NULL;
			_tmp13_ = _g_object_ref0 (engine_collection[engine_it]);
			engine = _tmp13_;
			{
				Block9Data* _data9_;
				gchar* language = NULL;
				...
				transname = _tmp25_;
				_tmp26_ = language;
				_tmp27_ = ibus_get_language_name (_tmp26_);
				_tmp28_ = _tmp27_;
				_tmp29_ = transname;
				...
				_data9_->e = _tmp35_;
				_tmp36_ = item;
				g_signal_connect_data (_tmp36_, "activate", (GCallback) _____lambda93__gtk_menu_item_activate, block9_data_ref (_data9_), (GClosureNotify) block9_data_unref, 0);
	...
	_tmp39_ = self->priv->m_ime_menu;
	gtk_widget_show_all ((GtkWidget*) _tmp39_);
	_tmp40_ = self->priv->m_ime_menu;
	gtk_menu_shell_set_take_focus ((GtkMenuShell*) _tmp40_, FALSE);
	_tmp41_ = display_backup;
	if (_tmp41_ != NULL) {
		GdkDisplayManager* _tmp42_;
		GdkDisplay* _tmp43_;
		_tmp42_ = gdk_display_manager_get ();
		_tmp43_ = display_backup;
		gdk_display_manager_set_default_display (_tmp42_, _tmp43_);
	}
...
static gboolean
__lambda106_ (Block10Data* _data10_)
{
	Panel* self;
	gboolean result;
	self = _data10_->self;
                    ...
                    gboolean all_update)
{
	...
	self->priv->m_property_icon_delay_time_id = g_timeout_add_full (G_PRIORITY_DEFAULT, timeout, ___lambda106__gsource_func, block10_data_ref (_data10_), block10_data_unref);
	block10_data_unref (_data10_);
	_data10_ = NULL;
}
	...
	_tmp74__length1 = self->priv->m_engines_length1;
	{
		...
		for (desc_it = 0; desc_it < desc_collection_length1; desc_it = desc_it + 1) {
			...
			desc = _tmp75_;
			{
				...
				_vala_array_add25 (&names, &names_length1, &_names_size_, _tmp78_);
