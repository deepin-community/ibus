/* keybindingmanager.c generated by valac 0.52.4, the Vala compiler
	...
	GdkModifierType result = 0U;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp3_ = mask;
				_tmp4_ = mask;
				if ((binding_mask & _tmp3_) == ((guint) _tmp4_)) {
					result = mask;
					return result;
				}
			}
	...
	g_return_if_fail (event != NULL);
	{
		...
		while (TRUE) {
			...
			if (_tmp5_ == GDK_KEY_PRESS) {
				...
				_tmp10_ = _tmp9_;
				keyval = _tmp10_->keyval;
				if (keyval >= ((guint) IBUS_KEY_A)) {
					_tmp12_ = keyval <= ((guint) IBUS_KEY_Z);
				} else {
					_tmp12_ = FALSE;
				}
				if (_tmp12_) {
					_tmp11_ = (modifiers & GDK_SHIFT_MASK) != ((guint) 0);
				} else {
					_tmp11_ = FALSE;
				}
				if (_tmp11_) {
				...
				_tmp13_ = self->priv->m_bindings;
				{
					GList* binding_collection = NULL;
					GList* binding_it = NULL;
					binding_collection = _tmp13_;
					for (binding_it = binding_collection; binding_it != NULL; binding_it = binding_it->next) {
						KeybindingManagerKeybinding* _tmp14_;
						KeybindingManagerKeybinding* binding = NULL;
						_tmp14_ = _keybinding_manager_keybinding_ref0 ((KeybindingManagerKeybinding*) binding_it->data);
						binding = _tmp14_;
						{
							...
							_tmp16_ = binding;
							_tmp17_ = keybinding_manager_keybinding_get_keysym (_tmp16_);
							_tmp18_ = _tmp17_;
							if (keyval != _tmp18_) {
								_tmp15_ = TRUE;
							} else {
								...
								_tmp21_ = _tmp20_;
								_tmp15_ = modifiers != ((guint) _tmp21_);
							}
							if (_tmp15_) {
								_keybinding_manager_keybinding_unref0 (binding);
								continue;
