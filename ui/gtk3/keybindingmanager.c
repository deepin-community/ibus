/* keybindingmanager.c generated by valac 0.56.12, the Vala compiler
                         ...
                         gpointer handler_target)
{
	...
	KeybindingManagerKeybinding* _tmp0_;
	KeybindingManagerKeybinding* _tmp1_;
	KeybindingManagerKeybinding* _tmp2_;
	gboolean result;
	g_return_val_if_fail (self != NULL, FALSE);
	if (!keybinding_manager_grab_keycode (self, keysym, (guint) modifiers)) {
		result = FALSE;
		return result;
	}
	_tmp0_ = keybinding_manager_keybinding_new (keysym, modifiers, handler, handler_target);
	binding = _tmp0_;
	_tmp1_ = binding;
	_tmp2_ = _keybinding_manager_keybinding_ref0 (_tmp1_);
	self->priv->m_bindings = g_list_append (self->priv->m_bindings, _tmp2_);
	result = TRUE;
	...
	_tmp0_ = self->priv->m_bindings;
	{
		...
		for (binding_it = binding_collection; binding_it != NULL; binding_it = binding_it->next) {
			...
			binding = _tmp1_;
			{
				...
				if (_tmp2_) {
					KeybindingManagerKeybinding* _tmp9_;
					guint _tmp10_;
					guint _tmp11_;
					...
					_tmp9_ = binding;
					_tmp10_ = keybinding_manager_keybinding_get_keysym (_tmp9_);
					_tmp11_ = _tmp10_;
					_tmp12_ = binding;
					_tmp13_ = keybinding_manager_keybinding_get_modifiers (_tmp12_);
					_tmp14_ = _tmp13_;
					keybinding_manager_ungrab_keycode (self, _tmp11_, (guint) _tmp14_);
					_tmp15_ = binding;
					_tmp16_ = _keybinding_manager_keybinding_ref0 (_tmp15_);
					remove_bindings = g_list_append (remove_bindings, _tmp16_);
				}
				_keybinding_manager_keybinding_unref0 (binding);
			}
		}
	}
	_tmp17_ = remove_bindings;
	{
		GList* binding_collection = NULL;
		GList* binding_it = NULL;
		binding_collection = _tmp17_;
		for (binding_it = binding_collection; binding_it != NULL; binding_it = binding_it->next) {
			KeybindingManagerKeybinding* _tmp18_;
			KeybindingManagerKeybinding* binding = NULL;
			_tmp18_ = _keybinding_manager_keybinding_ref0 ((KeybindingManagerKeybinding*) binding_it->data);
			binding = _tmp18_;
			{
				KeybindingManagerKeybinding* _tmp19_;
				_tmp19_ = binding;
				self->priv->m_bindings = vala_g_list_remove_full (self->priv->m_bindings, _tmp19_, _keybinding_manager_keybinding_unref0_);
				_keybinding_manager_keybinding_unref0 (binding);
			}
		}
	}
                                 ...
                                 guint modifiers)
{
	...
	xdisplay = _tmp3_;
	_tmp4_ = xdisplay;
	keycode = (gint) XKeysymToKeycode (_tmp4_, (gulong) keyval);
	if (keycode == 0) {
		g_warning ("keybindingmanager.vala:247: Can not convert keyval=%u to keycode!", keyval);
		result = FALSE;
		_g_object_unref0 (display);
		return result;
	}
	_tmp5_ = g_new0 (guchar, (XI_LASTEVENT + 7) / 8);
	memset (&_tmp6_, 0, sizeof (XIEventMask));
	_tmp6_.deviceid = XIAllMasterDevices;
	_tmp6_.mask = (g_free (_tmp6_.mask), NULL);
	_tmp6_.mask = _tmp5_;
	_tmp6_.mask_len = (XI_LASTEVENT + 7) / 8;
	evmask = _tmp6_;
	_tmp7_ = evmask;
	_tmp8_ = _tmp7_.mask;
	_tmp8__length1 = _tmp7_.mask_len;
	XISetMask (_tmp8_, XI_KeyPress);
	_tmp9_ = evmask;
	_tmp10_ = _tmp9_.mask;
	_tmp10__length1 = _tmp9_.mask_len;
	XISetMask (_tmp10_, XI_KeyRelease);
	_tmp11_ = xdisplay;
	_tmp12_ = xdisplay;
	_tmp13_ = evmask;
	_tmp15_ = keybinding_manager_get_grab_modifiers (self, modifiers, &_tmp14_);
	_tmp16_ = _tmp15_;
	_tmp16__length1 = _tmp14_;
	_tmp17_ = XIGrabKeycode (_tmp11_, XIAllMasterDevices, keycode, DefaultRootWindow (_tmp12_), (gint) GrabModeAsync, (gint) GrabModeAsync, TRUE, &_tmp13_, (gint) _tmp14_, _tmp16_);
	_tmp16_ = (_vala_XIGrabModifiers_array_free (_tmp16_, _tmp16__length1), NULL);
	retval = _tmp17_;
	result = retval == 0;
	 (&evmask);
	_g_object_unref0 (display);
                                   ...
                                   guint modifiers)
{
	...
	keycode = (gint) XKeysymToKeycode (_tmp4_, (gulong) keyval);
	if (keycode == 0) {
		g_warning ("keybindingmanager.vala:278: Can not convert keyval=%u to keycode!", keyval);
		result = FALSE;
		_g_object_unref0 (display);
		return result;
	}
	_tmp5_ = xdisplay;
	_tmp6_ = xdisplay;
	_tmp8_ = keybinding_manager_get_grab_modifiers (self, modifiers, &_tmp7_);
	_tmp9_ = _tmp8_;
	_tmp9__length1 = _tmp7_;
	_tmp10_ = XIUngrabKeycode (_tmp5_, XIAllMasterDevices, keycode, DefaultRootWindow (_tmp6_), (gint) _tmp7_, _tmp9_);
	_tmp9_ = (_vala_XIGrabModifiers_array_free (_tmp9_, _tmp9__length1), NULL);
	retval = _tmp10_;
	result = retval == 0;
	_g_object_unref0 (display);
	return result;
