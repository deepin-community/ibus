...
VALA_EXTERN gboolean is_running_daemon_via_systemd (GDBusConnection* connection,
                                        ...
                                        gboolean verbose);
VALA_EXTERN gboolean start_daemon_via_systemd (GDBusConnection* connection,
                                   gboolean restart,
                                   gboolean verbose);
...
gboolean
start_daemon_via_systemd (GDBusConnection* connection,
                          gboolean restart,
                          gboolean verbose)
{
	gchar* object_path = NULL;
...
exec_setxkbmap (IBusEngineDesc* engine)
{
	...
	}
	{
		...
		if (!_tmp34_) {
			...
			g_warning ("main.vala:267: Switch xkb layout to %s failed.", _tmp40_);
			result = EXIT_FAILURE;
			args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
			_g_free0 (standard_error);
			_g_free0 (option);
	...
	__catch0_g_spawn_error:
	{
		...
		g_warning ("main.vala:272: Execute setxkbmap failed: %s", _tmp42_);
		result = EXIT_FAILURE;
		_g_error_free0 (e);
		args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
		_g_free0 (standard_error);
	...
	if (exit_status != 0) {
		...
		g_warning ("main.vala:277: Execute setxkbmap failed: %s", _tmp44_);
		result = EXIT_FAILURE;
		args = (_vala_array_free (args, args_length1, (GDestroyNotify) g_free), NULL);
		_g_free0 (standard_error);
		_g_free0 (option);
...
start_daemon_real (gchar** argv,
                   ...
                   gboolean restart)
{
	static const GOptionEntry options[4] = {{"type", (gchar) 0, (gint) 0, G_OPTION_ARG_STRING, &daemon_type, N_ ("Start or restart daemon with \"direct\" or \"systemd\" TYPE."), "TYPE"}, {"service-file", (gchar) 0, (gint) 0, G_OPTION_ARG_STRING, &systemd_service_file, N_ ("Start or restart daemon with SYSTEMD_SERVICE file."), "SYSTEMD_SERVICE"}, {"verbose", (gchar) 0, (gint) 0, G_OPTION_ARG_NONE, &verbose, N_ ("Show debug messages."), NULL}, {NULL}};
	GOptionContext* option = NULL;
	GOptionContext* _tmp0_;
	GOptionContext* _tmp1_;
	GOptionContext* _tmp2_;
	...
	if (_tmp10_) {
		const gchar* _tmp13_;
		_tmp13_ = daemon_type;
		_tmp9_ = g_strcmp0 (_tmp13_, "systemd") != 0;
	} else {
		_tmp9_ = FALSE;
	}
	if (_tmp9_) {
		FILE* _tmp14_;
		_tmp14_ = stderr;
		fprintf (_tmp14_, "type argument must be \"direct\" or \"systemd\"\n");
		result = EXIT_FAILURE;
		_g_option_context_free0 (option);
		return result;
	}
	_tmp15_ = systemd_service_file;
	if (_tmp15_ == NULL) {
		gchar* _tmp16_;
		_tmp16_ = g_strdup (SYSTEMD_SESSION_GNOME_FILE);
		_g_free0 (systemd_service_file);
		systemd_service_file = _tmp16_;
	}
	{
		gboolean _tmp17_ = FALSE;
		_tmp17_ = TRUE;
		while (TRUE) {
			...
			_tmp19_ = get_session_bus (verbose);
			connection = _tmp19_;
			_tmp20_ = connection;
			if (_tmp20_ == NULL) {
			...
			if (restart) {
				...
				const gchar* _tmp25_;
				_tmp21_ = connection;
				_tmp22_ = get_ibus_systemd_object_path (_tmp21_, verbose);
				_g_free0 (object_path);
				object_path = _tmp22_;
				_tmp23_ = object_path;
				if (_tmp23_ == NULL) {
				...
				_tmp24_ = connection;
				_tmp25_ = object_path;
				if (!is_running_daemon_via_systemd (_tmp24_, _tmp25_, verbose)) {
					_g_free0 (object_path);
					_g_object_unref0 (connection);
					break;
				}
			...
			_tmp26_ = connection;
			if (!start_daemon_via_systemd (_tmp26_, restart, verbose)) {
				_g_free0 (object_path);
				_g_object_unref0 (connection);
				break;
			}
			...
			_tmp27_ = connection;
			_tmp28_ = get_ibus_systemd_object_path (_tmp27_, verbose);
			_g_free0 (object_path);
			object_path = _tmp28_;
			_tmp29_ = object_path;
			if (_tmp29_ == NULL) {
			...
			_tmp30_ = connection;
			_tmp31_ = object_path;
			if (!is_running_daemon_via_systemd (_tmp30_, _tmp31_, verbose)) {
				_g_free0 (object_path);
				_g_object_unref0 (connection);
				break;
			}
	...
	_tmp32_ = daemon_type;
	if (g_strcmp0 (_tmp32_, "systemd") == 0) {
		result = EXIT_FAILURE;
		_g_option_context_free0 (option);
		return result;
	}
	if (restart) {
		...
		if (_tmp34_ == NULL) {
			FILE* _tmp35_;
			_tmp35_ = stderr;
			fprintf (_tmp35_, "%s", _ ("Can't connect to IBus.\n"));
			result = EXIT_FAILURE;
			_g_object_unref0 (bus);
			_g_option_context_free0 (option);
			return result;
		...
		_tmp36_ = bus;
		ibus_bus_exit (_tmp36_, TRUE);
		if (verbose) {
			FILE* _tmp37_;
			_tmp37_ = stderr;
			fprintf (_tmp37_, "%s", "Succeed to restart ibus-daemon with an IBus API " "directly.\n");
		}
		_g_object_unref0 (bus);
	} else {
		gchar* startarg = NULL;
		...
		const gchar* _tmp53_;
		_tmp38_ = g_strdup ("ibus-daemon");
		startarg = _tmp38_;
		_tmp39_ = startarg;
		_tmp40_ = g_strdup (_tmp39_);
		_g_free0 (argv[0]);
		argv[0] = _tmp40_;
		_tmp41_ = g_getenv ("PATH");
		_tmp43_ = _tmp42_ = g_strsplit (_tmp41_, ":", 0);
		paths = _tmp43_;
		paths_length1 = _vala_array_length (_tmp42_);
		_paths_size_ = paths_length1;
		_tmp44_ = paths;
		_tmp44__length1 = paths_length1;
		{
			gchar** path_collection = NULL;
			gint path_collection_length1 = 0;
			gint _path_collection_size_ = 0;
			gint path_it = 0;
			path_collection = _tmp44_;
			path_collection_length1 = _tmp44__length1;
			for (path_it = 0; path_it < path_collection_length1; path_it = path_it + 1) {
				...
				path = path_collection[path_it];
				{
					...
					full_path = _tmp47_;
					_tmp48_ = full_path;
					if (g_file_test (_tmp48_, G_FILE_TEST_IS_EXECUTABLE)) {
						const gchar* _tmp49_;
						gchar* _tmp50_;
						_tmp49_ = full_path;
		...
		if (verbose) {
			...
			_tmp51_ = stderr;
			_tmp52_ = startarg;
			fprintf (_tmp51_, "Running \"%s\" directly as a foreground " "process.\n", _tmp52_);
		}
		_tmp53_ = startarg;
		execv (_tmp53_, argv);
